...
<? if ($zone['expansion'] <= $expansion_limit): ?>
            <option value="index.php?editor=<?=$curreditor?>&z=<?=$zone['short_name']?>&zoneid=<?=$zone['id']?>"<?echo ($currzoneid == $zone['id']) ? " selected" : "";?>><?=$zone['short_name']?>(<?=$zone['version']?>) [<?=$zone['zoneidnumber']?>] </option>
<? endif;?>
<? endforeach;?>
          </select>
          &nbsp; and &nbsp;
...
<? foreach ($npcs as $npc): ?>
            <option value="index.php?editor=<?=$curreditor?>&z=<?=$currzone?>&zoneid=<?=$currzoneid?>&npcid=<?=$npc['id']?>"<?echo ($currnpc == $npc['id']) ? " selected" : "";?>><?=$npc['name']?>[<?=$npc['id']?>]</option>
<? endforeach;?>
          </select>
        </td>
