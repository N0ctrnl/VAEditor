...
<?php if ($zone['expansion'] <= $expansion_limit): ?>
                <option value="index.php?editor=<?=$curreditor?>&z=<?=$zone['short_name']?>&zoneid=<?=$zone['id']?>"<?php if ($currzoneid == $zone['id']): ?> selected<?php endif;?>><?=$zone['long_name']?>(<?=$zone['version']?>) [<?=$zone['zoneidnumber']?>] </option>
<?php endif;?>
<?php endforeach;?>
              </select>
            </td>
